...
type ReactionType = "thumbs_up" | "thumbs_down" | "rocket" | "sad" | "heart";
...
> = {
  ...
  thumbs_down: {
    color:
      "text-red-500 hover:text-red-700 dark:text-red-300 dark:hover:text-red-200",
    icon: (
      <svg
        className=" w-4 h-4"
        fill="none"
  ...
  rocket: {
    color:
      "text-green-500 hover:text-green-700 dark:text-green-300 dark:hover:text-green-200",
    icon: (
      <svg
        className=" w-4 h-4"
        fill="none"
  ...
  sad: {
    color:
      "text-purple-500 hover:text-purple-700 dark:text-purple-300 dark:hover:text-purple-200",
    icon: (
      <svg
        className=" w-4 h-4"
        fill="none"
  ...
  heart: {
    color:
      "text-pink-500 hover:text-pink-700 dark:text-pink-300 dark:hover:text-pink-200",
    icon: (
      <svg
        className=" w-4 h-4"
        fill="none"
...
const ReactionsCounter = ({
  ...
  const sortedReactions = useMemo(() => {
    if (floating) {
      return allReactionTypes;
    }
    return [...allReactionTypes]
      .filter((type) => (aggregate?.[type] ?? 0) > 0)
  ...
  return (
    ...
    >
      {sortedReactions.map((reactionType) => {
        ...
        const count = aggregate?.[reactionType];
        const isActive = user?.[reactionType];

        return (
          <button
            key={reactionType}
            className={classNames(
              "flex items-center space-x-1 text-sm hover:bg-gray-100 dark:hover:bg-gray-900 p-2 rounded-md",
              config.color,
              isActive
          ...
          >
            {config.icon}
            {!floating && count ? <span>{count}</span> : null}
          </button>
        );
      })}
    </div>
